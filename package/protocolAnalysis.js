...
const ProtocolUtil = require("../utils/protocolUtil.js");
const BytesHexStrUtil = require("../utils/bytesHexStr.js");
const logger = require("../modules/logger").logger("info");

// 解析协议
module.exports = function protocolAnalysis(str) {
...
function analysisCMDModelList(data, str) {
  ...
  while (read + 8 < data.length) {
    // 解决连包问题，并且过滤掉两端的无效数据
    if ((data[read] & 0xff) != Cmd_const.CMD_Head) {
      // 不等于0xAB
      read++;
      continue;
    }
    ...
    switch (item.cmdKey) {
      ...
      case Cmd_const.CMD_Services_getAddresses: // address未解析
        ...
        if (cmdValue.length > 8) {
          let address = BytesHexStrUtil.bytes2StringByUTF_8(
            cmdValue.slice(8, cmdValue.length)
          );
      ...
      case Cmd_const.CMD_Services_getLocationGSM:
        const ev07b_gsm = data2Model_GSM(
          cmdValue,
          Cmd_const.CMD_Services_getLocationGSM
        );
        servicesData[Prot_const.Services_0x21] = ev07b_gsm;
        break;
      // WIFI定位
      case Cmd_const.CMD_Services_getLocationWIFI:
        const ev07b_wifi = data2Model_Wifi(
          cmdValue,
          Cmd_const.CMD_Services_getLocationWIFI
        );
...
function analysisResponseData(cmdModel) {
  ...
  switch (cmdKey) {
    ...
    case Cmd_const.CMD_Response_KeyLengthError:
      responseData[Prot_const.Response_Msg] =
        Prot_const.Response_KeyLengthError;
      break;
    case Cmd_const.CMD_Response_DataFormatError:
      responseData[Prot_const.Response_Msg] =
        Prot_const.Response_DataFormatError;
      break;
    ...
    case Cmd_const.CMD_Response_ParameterError:
      responseData[Prot_const.Response_Msg] =
        Prot_const.Response_ParameterError;
      break;
    ...
    case Cmd_const.CMD_Response_NoPasswordVerify:
      responseData[Prot_const.Response_Msg] =
        Prot_const.Response_NoPasswordVerify;
      break;
...
function getCmdDataObject(cmdModel) {
  if (
    !cmdModel?.cmd_bodyData &&
    !cmdModel?.cmd_headData &&
    !cmdModel?.originalData
  )
    return {};
  let { cmd_bodyData, cmd_headData, originalData } = cmdModel;
  const cmdHeadData = {}; // 创建一个键值对的对象
  const cmdBodyData = {}; // 身体键值对对象
  // @ts-ignore
  const cmdData = {};
  ...
  cmdDataList.forEach((item) => {
    ...
    switch (item.cmdKey) {
      case Cmd_const.CMD_Data_IMEI:
        locationData[Prot_const.Data_0x01] =
          BytesHexStrUtil.bytes2StringByASCII(cmdValue);
        break;
      ...
      case Cmd_const.CMD_Services_ICCID:
        locationData[Prot_const.Data_0x04] =
          BytesHexStrUtil.bytes2StringByASCII(cmdValue);
        break;
...
function getDeviceStatus(status) {
  ...
  if (isBLE) {
    ...
    deviceStatus[Prot_const.Data_0x24_DataType] =
      Prot_const.Data_DataType_Smart;
  } else if (isBeacon) {
    deviceStatus[Prot_const.Data_0x24_DataType] =
      Prot_const.Data_DataType_Beacon;
  } else if (isHomeWifi) {
    deviceStatus[Prot_const.Data_0x24_DataType] =
      Prot_const.Data_DataType_HomeWifi;
  } else if (isGPS) {
    deviceStatus[Prot_const.Data_0x24_DataType] = Prot_const.Data_DataType_GPS;
  } else if (isWIFI && isGMS) {
    deviceStatus[Prot_const.Data_0x24_DataType] =
      Prot_const.Data_DataType_WIFIGSM;
  } else if (isWIFI) {
...
function getJson_Wifi2(data) {
  ...
  for (let i = 0; i + 7 < data.length;) {
    ...
    const name = BytesHexStrUtil.bytes2StringByUTF_8(
      data.slice(i + 8, i + 8 + nameLength)
    );
...
function data2Model_BLE(data) {
  if (data.length > 1) {
    ...
    if (data.length > 14) {
      const description = BytesHexStrUtil.bytes2StringByUTF_8(
        data.slice(14, data.length)
      );
...
function data2Model_Beacon(data) {
  ...
  if (descriptionStatus == 1) {
    let description = BytesHexStrUtil.bytes2StringByUTF_8(
...
function getAlarmStatus(alarmFlag) {
  ...
  alarmStatus[Prot_const.Data_0x02_BatteryLow] = ProtocolUtil.getBit(
    alarmFlag,
    0,
    1
  );
  alarmStatus[Prot_const.Data_0x02_OverSpeed] = ProtocolUtil.getBit(
    alarmFlag,
    1,
    2
  );
  alarmStatus[Prot_const.Data_0x02_FallDown] = ProtocolUtil.getBit(
    alarmFlag,
    2,
    3
  );
  ...
  alarmStatus[Prot_const.Data_0x02_PowerOff] = ProtocolUtil.getBit(
    alarmFlag,
    8,
    9
  );
  alarmStatus[Prot_const.Data_0x02_PowerOn] = ProtocolUtil.getBit(
    alarmFlag,
    9,
    10
  );
  alarmStatus[Prot_const.Data_0x02_Motion] = ProtocolUtil.getBit(
    alarmFlag,
    10,
    11
  );
  alarmStatus[Prot_const.Data_0x02_NoMotion] = ProtocolUtil.getBit(
    alarmFlag,
    11,
    12
  );
  alarmStatus[Prot_const.Data_0x02_SosKey] = ProtocolUtil.getBit(
    alarmFlag,
    12,
    13
  );
  ...
  alarmStatus[Prot_const.Data_0x02_Side1key] = ProtocolUtil.getBit(
    alarmFlag,
    13,
    14
  );
  alarmStatus[Prot_const.Data_0x02_Side2key] = ProtocolUtil.getBit(
    alarmFlag,
    14,
    15
  );
  alarmStatus[Prot_const.Data_0x02_BatteryCharging] = ProtocolUtil.getBit(
    alarmFlag,
    15,
    16
  );
  alarmStatus[Prot_const.Data_0x02_BatteryNoCharging] = ProtocolUtil.getBit(
    alarmFlag,
    16,
    17
  );
  alarmStatus[Prot_const.Data_0x02_AlarmEnd] = ProtocolUtil.getBit(
    alarmFlag,
    17,
    18
  );
  alarmStatus[Prot_const.Data_0x02_Amber] = ProtocolUtil.getBit(
    alarmFlag,
    18,
    19
  );
  alarmStatus[Prot_const.Data_0x02_Welfare] = ProtocolUtil.getBit(
    alarmFlag,
    19,
    20
  );
  alarmStatus[Prot_const.Data_0x02_AmberEnd] = ProtocolUtil.getBit(
    alarmFlag,
    20,
    21
  );

  alarmStatus[Prot_const.Data_0x02_FallDownEnd] = ProtocolUtil.getBit(
    alarmFlag,
    21,
    22
  );
  alarmStatus[Prot_const.Data_0x02_WelfareEnd] = ProtocolUtil.getBit(
    alarmFlag,
    22,
    23
  );
  alarmStatus[Prot_const.Data_0x02_Upload] = ProtocolUtil.getBit(
    alarmFlag,
    23,
    24
  );
  alarmStatus[Prot_const.Data_0x02_HomeFenceOut] = ProtocolUtil.getBit(
    alarmFlag,
    24,
    25
  );
  alarmStatus[Prot_const.Data_0x02_HomeFenceIn] = ProtocolUtil.getBit(
    alarmFlag,
    25,
    26
  );
  // GEO报警的进/出状态位
  alarmStatus[Prot_const.Data_0x02_Geo1_In] = ProtocolUtil.getBit(
    alarmFlag,
    26,
    27
  );
  alarmStatus[Prot_const.Data_0x02_Geo2_In] = ProtocolUtil.getBit(
    alarmFlag,
    27,
    28
  );
  alarmStatus[Prot_const.Data_0x02_Geo3_In] = ProtocolUtil.getBit(
    alarmFlag,
    28,
    29
  );
  alarmStatus[Prot_const.Data_0x02_Geo4_In] = ProtocolUtil.getBit(
    alarmFlag,
    29,
    30
  );
  alarmStatus[Prot_const.Data_0x02_BleDisConnected] = ProtocolUtil.getBit(
    alarmFlag,
    30,
    31
  );
...
function getAlarmStatusExtend(alarmFlag) {
  ...
  alarmExtendStatus[Prot_const.Data_0x02_FallOff] = ProtocolUtil.getBit(
    alarmFlag,
    0,
    1
  );
...
//module.exports = protocolAnalysis;
